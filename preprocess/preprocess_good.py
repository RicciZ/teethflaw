...
def to_matrix(params):
    ...
    M[:3, 3] = params[4:]
    M[:3, :3] = R.from_quat(params[0:4]).as_dcm()
    M[3, 3] = 1
...
def _preprocess_good(folder, root, save_dir):
    x_path = glob.glob(os.path.join(root, folder, 'tooth*.stl'))[0]
...
def preprocess(root, save_dir, n_thread=4):
    ...
    with Pool(n_thread) as p:
        list(tqdm.tqdm(p.imap_unordered(partial(_preprocess, root=root, save_dir=save_dir), folders),
                       total=len(folders)))

def preprocess_good(root, save_dir, n_thread=4):
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)
    ...
    folders = [f for f in os.listdir(root) if f.startswith('C')]
    with Pool(n_thread) as p:
        list(tqdm.tqdm(p.imap_unordered(partial(_preprocess_good, root=root, save_dir=save_dir), folders),
                       total=len(folders)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save', type=str, default='../std_data',
                        help='path to save')
    parser.add_argument('--id', type=int, default=36,
                        help='tooth id')
    ...
    bad_path = os.path.join(args.data, 'bad')
    good_path = os.path.join(args.data, 'good')
    if args.all:
        args.id = [11,12,13,14,15,16,17,21,22,23,24,25,26,27,28,31,32,33,34,35,36,37,38,41,42,43,44,45,46,47,48]
        for id in args.id:
            preprocess(os.path.join(bad_path, str(id)), os.path.join(args.save, str(id)))
            preprocess_good(os.path.join(good_path, str(id)), os.path.join(args.save, str(id)))
    else:
        preprocess(os.path.join(args.bad_path, str(args.id)), os.path.join(args.save, str(args.id)))
        preprocess_good(os.path.join(good_path, str(args.id)), os.path.join(args.save, str(args.id)))
